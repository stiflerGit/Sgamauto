...
class MQ():
    ...
    RL_VALUE                     = 200#5        # define the load resistance on the board, in kilo ohms
    RO_CLEAN_AIR_FACTOR          = 60#9.83     # RO_CLEAR_AIR_FACTOR=(Sensor resistance in clean air)/RO,
    ...
    def __init__(self, Ro=0, analogPin=0):
        self.Ro = Ro
        self.MQ_PIN = analogPin
        self.adc = Adafruit_MCP3008.MCP3008(spi=SPI.SpiDev(0, 0))#MCP3008()

        self.LPGCurve = [-1, 0.35, -0.63]    # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent"
                                            # to the original curve. 
                                            # data format:{ x, y, slope}; point1: (lg200, 0.21), point2: (lg10000, -0.59) 
        self.COCurve = [-1, 0.35, -0.63]     # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg200, 0.72), point2: (lg10000,  0.15)
        self.SmokeCurve =[-1, 0.35, -0.63]   # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg200, 0.53), point2: (lg10000,  -0.22)
        self.AlcCurve =[-1, 0.35, -0.63]
    ...
    def MQResistanceCalculation(self, raw_adc):
        return float(self.RL_VALUE*(1023.0 - (raw_adc + 1))/ float(raw_adc+1));
    ...
    def MQCalibration(self, mq_pin):
        ...
        for i in range(self.CALIBARAION_SAMPLE_TIMES):          # take multiple samples
            val += self.MQResistanceCalculation(self.adc.read_adc(mq_pin))
